  ...
  describe '#self.load_file' do
    context 'for a typical kubeadm admin.conf' do
      ...
      it "loads the current context" do
        ...
        expect(subject.context.user).to eq 'kubernetes-admin'
      end

      it "loads the correct cluster" do
        expect(subject.cluster.server).to eq 'https://192.168.100.100:6443'
      end

      it "loads the correct user" do
        expect(subject.user.client_certificate_data).to match /^[a-zA-Z0-9.=]+$/
        expect(subject.user.client_key_data).to match /^[a-zA-Z0-9.=]+$/
      end
    end
